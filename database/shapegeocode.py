...
class geocoder:
    ...
    def __init__(self, shp_src, filters=None):
        """
        create a new geocode using the specified shapefile
        """
        self.shp_src = shp_src
        self._init_polygons(filters)

    def _init_polygons(self, filters=None):
        ...
        for i in range(len(recs)):
            ...
            for j in range(len(fields)):
                rec[fields[j]] = _rec[j]
            if filters and not filters(rec):
                continue
    ...
    def geocode(self, lat, lon, filters=None, max_dist=0.0):
        """
...
def _shape_to_polygon(shp):
    ...
    poly = []
    xrange = (float('inf'), float('inf') * -1)
    yrange = (float('inf'), float('inf') * -1)
